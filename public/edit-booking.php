...
<div class="container">
    ...
    <?php if (!empty($message)): ?>
        <div class="alert alert-<?= strpos($message, 'success') !== false
          ? 'success'
          : 'danger' ?>">
...
<form method="post">
    ...
    <input type="date" id="check_in" name="check_in" value="<?= htmlspecialchars(
      $booking['check_in']
    ) ?>" required min="<?= date('Y-m-d') ?>">

    <label for="check_out">Дата виїзду:</label>
    <input type="date" id="check_out" name="check_out" value="<?= htmlspecialchars(
      $booking['check_out']
    ) ?>" required min="<?= date('Y-m-d') ?>">

    <label for="status">Статус:</label>
    <select id="status" name="status">
        <option value="Очікує" <?= $booking['status'] == 'Очікує'
          ? 'selected'
          : '' ?>>Очікує підтвердження</option>
        <option value="Готово" <?= $booking['status'] == 'Готово'
          ? 'selected'
          : '' ?>>Підтверджено</option>
        <option value="Скасовано" <?= $booking['status'] == 'Скасовано'
          ? 'selected'
          : '' ?>>Скасовано</option>
    </select>
<label for="admin_comment">Коментар менеджера:</label>
<textarea id="admin_comment" name="admin_comment" class="form-control"><?= htmlspecialchars(
