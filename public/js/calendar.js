;( function( $, window, undefined ) {	
	...
	// the options
	$.Calendario.defaults = {
		weeks : [ 'Понеділок', 'Вівторок', 'Середа', 'Четверг', 'Пятниця', 'Субота', 'Неділя' ],
...
$(document).ready(function () {
    ...
    $('.fc-date').each(function () {
        ...
        if (bookedDates.has(fullDate)) {
            ...
            $container.addClass('booked-date').attr('title', 'Ця дата вже заброньована');
        } else {
    ...
    function clearPreviousSelection() {
        $('.fc-date').parent().removeClass('selected-range selected-start selected-end');
    }
