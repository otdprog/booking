...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  if (
    !isset($_POST['booking_token']) ||
    $_POST['booking_token'] !== $_SESSION['booking_token']
  ) {
    $response = [
      'status' => 'danger',
      'message' => 'Невірний запит бронювання.',
    ];
  } else {
...
<div class="content-wrapper">
    ...
    <div class="room-description mt-3">
        ...
        <p class="deskription"><?= nl2br(
          htmlspecialchars($room['description'] ?? 'Опис відсутній')
        ) ?></p>
    </div>
</div>

            <div class="col-md-6">
                <h3 class="hello"><p><strong>Будиночок:</strong> <?= htmlspecialchars(
                  $room['room_number']
                ) ?></p></h3>
                <h3 class="hello"><p><strong>Тип будиночка:</strong> <?= htmlspecialchars(
                  $room['room_type'] ?? 'Не вказано'
                ) ?></p></h3>
                <h3 class="hello"><p><strong>Ціна:</strong> <?= htmlspecialchars(
                  $room['price']
                ) ?>грн За добу</p></h3>
                
                <form method="post">
                    <input type="hidden" name="room_id" value="<?= htmlspecialchars(
                      $room['id']
                    ) ?>">
                    <input type="hidden" name="booking_token" value="<?= $_SESSION[
                      'booking_token'
                    ] ?>">
