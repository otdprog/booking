...
class RoomController
{
      ...
      public function addRoom($data, $files)
      {
            ...
            if ($roomExists) {
                  $_SESSION['message'] =
                        "Помилка: Будиночок із таким номером вже існує!";
                  header("Location: admin.php");
                  exit();
            }
            $roomId = $this->roomDAO->addRoom(
                  $data['room_number'],
                  $data['room_type'],
                  $data['price'],
                  $data['description']
      ...
      private function uploadImages($files)
      {
            ...
            if (!file_exists($targetDir) && !mkdir($targetDir, 0777, true)) {
                  die(
                        "Помилка створення директорії для завантаження: " .
                              $targetDir
                  );
            }
            ...
            foreach ($files['tmp_name'] as $key => $tmpName) {
                  if ($files['error'][$key] === UPLOAD_ERR_OK) {
                        $fileName =
                              uniqid() . "_" . basename($files['name'][$key]);
                        ...
                        die(
                              "Помилка завантаження файлу (код): " .
                                    $files['error'][$key]
                        );
                  }
            }
      ...
      public function updateRoom($data, $files)
      {
            ...
            $success = $this->roomDAO->updateRoom(
                  $data['room_id'],
                  $data['room_number'],
                  $data['room_type'],
                  $data['price'],
            ...
            if (
                  $success &&
                  isset($files['new_images']) &&
                  !empty($files['new_images']['name'][0])
            ) {
                  $imagePaths = $this->uploadImages($files['new_images']);
                  if (!empty($imagePaths)) {
                        $this->roomDAO->addRoomImages(
                              $data['room_id'],
                              $imagePaths
                        );
                  }
      ...
      public function deleteRoomImage($imageId)
      {
            ...
            if ($image) {
                  $imagePath =
                        __DIR__ . "/../../public/" . $image['image_path'];
