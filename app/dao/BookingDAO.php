...
class BookingDAO
{
     ...
     public function getFilteredBookings(
          ...
          if ($guestContact) {
               $query .=
                    " AND (bookings.guest_email LIKE :guest OR bookings.guest_phone LIKE :guest)";
          }
     ...
     public function confirmBooking($bookingId)
     {
          ...
          return $updateStmt->execute([$bookingId])
               ? true
     ...
     public function updateBooking(
          ...
          $sql =
               "UPDATE bookings SET check_in = ?, check_out = ?, status = ?, admin_comment = ? WHERE id = ?";
          $stmt = $this->pdo->prepare($sql);
          return $stmt->execute([
               $checkIn,
               $checkOut,
     ...
     {
          $sql =
               "SELECT check_in, check_out FROM bookings WHERE room_id = ? AND status = 'Готово'";
          $stmt = $this->pdo->prepare($sql);
          $stmt->execute([$roomId]);
